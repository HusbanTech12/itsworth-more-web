...
const testimonials = [
  {
    ...
    text: "Highly recommend CashingTech.com. Easy to complete the sale and I received a much better value for my old iPhone 5s than any other offer. Communication was clear and timely. I will definitely use them again.",
    rating: 5,
  ...
  },
  {
    ...
    text: "Strongly Recommend CashingTech.com. Very clear and simple process. Offer was confirmed on receipt of the iPad Pro and payment was promptly made. I received $150 more than using Apple's trade-in program.",
    rating: 5,
  ...
  return (
    <section className="py-20 lg:py-28 overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h2 className="text-3xl sm:text-4xl font-bold text-white mb-6 font-display leading-tight">
          Don&apos;t just take our word for it
        </h2>

        <div className="relative">
          <div className="min-h-[260px] flex flex-col items-center justify-center" key={current}>
            <div className="w-16 h-16 rounded-full overflow-hidden ring-4 ring-white/10 mb-5 animate-fade-in">
              <img
                src={t.avatar}
                alt={t.name}
            ...
            <blockquote className="mt-5 text-base sm:text-lg text-white/70 leading-relaxed max-w-xl mx-auto italic animate-fade-in" style={{ animationDelay: "0.15s" }}>
              &ldquo;{t.text}&rdquo;
            </blockquote>
            <p className="mt-5 font-semibold text-white animate-fade-in" style={{ animationDelay: "0.2s" }}>
              {t.name}
            </p>
            <p className="text-sm text-white/50 animate-fade-in" style={{ animationDelay: "0.25s" }}>{t.location}</p>
          </div>

          <div className="flex items-center justify-center gap-4 mt-8">
            <button
              onClick={prev}
              className="rounded-full p-2 text-white/50 hover:text-lime hover:bg-white/10 transition-colors"
              aria-label="Previous"
            >
              ...
              {testimonials.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    i === current ? "bg-lime w-6" : "bg-white/20 w-2 hover:bg-white/40"
                  }`}
                />
              ))}
            </div>
